...
class PurchaseOrderList(Resource):
    ...
    def post(self):
        ...
        if not SupplierService.get_by_id(data['supplier_id']):
            return api_response(message='Supplier not found', status_code=400)

        # Validate warehouse exists
        if not WarehouseService.get_by_id(data['warehouse_id']):
            return api_response(message='Warehouse not found', status_code=400)
